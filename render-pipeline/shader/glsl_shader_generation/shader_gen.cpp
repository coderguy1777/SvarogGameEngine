...
void ShaderGeneration::send_shaders() {
    ...
    for(unsigned int i = 0; i < vertex_shader_code.size(); i++) { 
        code_a += code_a + vertex_shader_code.get(i);
    }
    ...
    mat_shader_prg.material_vert_shader.set_code(get_vertex_code());
    mat_shader_prg.material_frag_shader.set_code(get_fragment_code());
